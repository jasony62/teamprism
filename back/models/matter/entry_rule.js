...
class Rule extends Base {
  ...
  async checkGroup(userid) {
    ...
    if (!this.group) return '没有指定作为进入规则的分组活动，请联系活动的组织者解决'
    let dmGroup = this.model('matter/group')
    let group = await dmGroup.byId(this.group.id)
    if (false === group || group.state !== 1) return '指定的分组活动不可访问，请联系活动的组织者解决。'
    ...
    if (grpRecs && grpRecs.length) {
      ...
      if (this.group.team && this.group.team.id) {
        if (grpUsr.team_id === this.group.team.id) bMatched = true
  ...
  async checkEnroll(userid) {
    ...
    if (!this.enroll || !this.enroll.id) return '没有指定作为进入规则的记录活动，请联系活动的组织者解决。'

    let dmEnroll = this.model('matter/enroll')
    let enroll = await dmEnroll.byId(this.enroll.id)
    if (false === enroll || enroll.state !== 1) return '指定作为进入规则的记录活动不存在，请联系活动的组织者解决。'
   ...
   */
  async check(userid) {
    if (!this.rule.scope) return true
