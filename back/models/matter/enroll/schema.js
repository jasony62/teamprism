...
class Schema extends DbModel {
     ...
     * @return object $oApp
     */
	async function setDynaSchemas(oApp, oTask = null) {
        if (!oApp.appRound) {
            let modelRnd = new Round()
            let oAppRound = modelRnd.getActive(oApp, {'fields' : 'id,rid,title,start_at,end_at,mission_rid'})
        } else {
            let oAppRound = oApp.appRound;
        }
