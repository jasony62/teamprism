...
class Matter extends DbModel {
  ...
  async byChannel(channel, { page = 1, size = 12 } = {}) {
    let sqlWhere = [['fieldMatch', 'channel_id', '=', channel.id]]
    let sqlParts = {
    ...
    if (chanMatters && chanMatters.length) {
      ...
      chanMatters.forEach(cm => {
        ...
        if (typedMatter) Object.assign(cm, typedMatter)
      })
    }
  ...
  async countByChannel(channel) {
    let sqlWhere = [['fieldMatch', 'channel_id', '=', channel.id]]
    let result = await this.selectOne('count(*) cnt', sqlWhere)

    if (result && result.cnt) return result.cnt
